...
const jobDetails = {
  ...
  createdAt: new Date(
    "Thu Jul 04 2024 16:28:33 GMT+0530 (India Standard Time)"
  ),
  creatorId: "clwu51z1r0000o69jrzowsuzi",
  description:
    "lksjhjkdhkjsdhjkhdskjhdjkh kkjhjkd kjdhkjd hskjhdjkh dkjshkjdh kjdhjkdhskjhdkjhdkjhdkjhdk jdjk djkh djkhdkjhdjksh kdjhkjdshkjds hkjdshkjdshjkd jdh skjhds jkdhs kjh djkh djkh dsjkh dkjh dkjhdjk hdjkhdskj",
  experience: "Fresher",
  ...
  title: "Nextjs",
  updatedAt: new Date(
    "Tue Jul 16 2024 16:48:04 GMT+0530 (India Standard Time)"
  ),
};
    ...
    >
      <div className={`${cardBg} h-3/4  rounded-xl p-4 space-y-4`}>
        ...
        <div className="grid grid-cols-3 gap-x-3 gap-y-4">
          {skills2.slice(0, 5).map((skill: string, index: number) => (
            <div
              className="pl-3 pr-3 pt-1 pb-1 border-2 border-opacity-30 border-[#546e7a] rounded-xl text-black text-center hover:rounded-md ease-in-out duration-300 font-medium"
              key={index}
            >
              {skill}
            </div>
          ))}
          <div className="pl-3 pr-3 pt-1 pb-1 border-2 border-opacity-30 border-[#546e7a] rounded-xl  text-center hover:rounded-md ease-in-out duration-300 font-medium text-black">
            {experience}
          </div>
        </div>
      </div>
      <div className="h-1/4 rounded-xl pl-4 pr-4 flex justify-between pt-7">
        ...
        <div className="mt-2 transition-transform duration-500 transform hover:rotate-3 hover:cursor-pointer ease-in-out hover:ease-in-out">
          <Link href={`jobs/${slug}`} prefetch={true}>
            <span className="pl-6 pr-6 pt-3 pb-3 bg-black dark:bg-gray-200 rounded-2xl text-gray-200 dark:text-black font-medium">
              Details
            </span>
          </Link>
        </div>
